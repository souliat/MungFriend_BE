...
//            message.setText(text);
    ...
    public static String setContent(String receiverNickname, MailType type){
        ...
        if (type == MailType.NORMAL_SIGNUP){
            ...
            sb.append("<h3>")
                    .append("[매칭 후 이용 안내]").append("</h3>\n");
            sb.append("<h3>")
                    .append("메인 페이지 우측 상단의 말풍선을 클릭하여 매칭된 상대방과의 채팅을 통해 상세 일정을 조율해주세요.😉").append("</h3>\n");
            sb.append("<h3>")
                    .append("개인 사정 상 산책을 못하게 될 경우 채팅을 통해 멍친구에게 사전에 반드시 알려주세요.😥").append("</h3>\n");
            sb.append("<br>");
            sb.append("<h3>")
                    .append("[산책 시 유의 사항]").append("</h3>\n");
            sb.append("<h3>")
                    .append("‼산책은 반드시 신청자 본인이 직접 해주세요.").append("</h3>\n");
            sb.append("<h3>")
                    .append("‼산책시켜 줄 멍멍이의 멍프로필을 클릭하여 특이 사항을 꼭 읽어주세요.").append("</h3>\n");
            sb.append("<h3>")
                    .append("‼멍멍이가 산책 중 아무거나 먹지 않도록 주의해주세요.").append("</h3>\n");
            sb.append("<h3>")
                    .append("‼멍멍이의 안전과 사고 예방을 위해 리드줄을 꼭 착용해주세요.").append("</h3>\n");
            sb.append("<h3>")
                    .append("‼산책 시 멍멍이가 배변을 하면 꼭 배변 봉투에 담아 처리를 해주세요.").append("</h3>\n");
            sb.append("<h3>")
                    .append("‼멍멍이가 입질이 심한 경우 입마개 착용을 권장합니다.").append("</h3>\n");
            sb.append("<a href='https://mungfriend.com' style=\"text-decoration-line:none\">🐶 멍친구로 바로가기</a>\n");
